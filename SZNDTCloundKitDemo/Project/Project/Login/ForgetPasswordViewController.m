...
@property (weak, nonatomic) IBOutlet UITextField *userTF;  // 手机号输入框
...
- (IBAction)getVerifyCodeBtnAction:(UIButton *)sender {
    ...
    if (![_userTF.text isMobileNumber]) {
...
- (void)getVerifyCodeNumber
{
     ...
     * 重设密码获取验证码，传入用户名
     */
    [[DTCloudManager defaultJNI_iOS_SDK]getForgetPasswordCodeWithUsername:self.userTF.text successCallback:^(NSDictionary *dic) {
...
- (void)confirmActionStart
{
     ...
     * 重设密码，传入用户、密码、验证码
     */
    [[DTCloudManager defaultJNI_iOS_SDK]resetPasswordWithUsername:self.userTF.text password:self.passwordTF.text securityCode:self.verifyTF.text successCallback:^(NSDictionary *dic) {
        
        [SVProgressHUD showSuccessWithStatus:dic[@"errmsg"]];
        
        [[NSUserDefaults standardUserDefaults]setValue:self.userTF.text forKey:@"DTPHONE"];
