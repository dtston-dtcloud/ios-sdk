...
- (APNumberButton *)numberButton:(int)number {
    ...
    if (number<7&&number!=0) {
        NSArray *ary=@[@"A",@"B",@"C",@"D",@"E",@"F"];
        [b setTitle:ary[number-1]  forState:UIControlStateNormal];
    }
    else{
        [b setTitle:[NSString stringWithFormat:@"%d", (number-6)>0?(number-6):0] forState:UIControlStateNormal];
    }
    return b;
}
